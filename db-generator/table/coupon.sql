...
CREATE TABLE `t_coupon` (
  ...
  `coupon_name` varchar(63) NOT NULL COMMENT '优惠券名称',
  `coupon_desc` varchar(127) DEFAULT '' COMMENT '优惠券介绍，通常是显示优惠券使用限制文字',
  `platform` tinyint(1) DEFAULT NULL COMMENT '使用平台：0->全部；1->移动；2->PC',
  `tag` varchar(63) DEFAULT '' COMMENT '优惠券标签，例如新人专用',
  `type` smallint(6) DEFAULT 0 COMMENT '优惠券赠送类型，0->全场赠券；1->会员赠券；2->购物赠券；3->注册赠券',
  `count` int(11) NOT NULL DEFAULT 0 COMMENT '优惠券总数量，如果是0，则是无限量',
  `amount` decimal(10,2) DEFAULT 0.00 COMMENT '优惠金额',
  `per_limit` int(11) DEFAULT 1 COMMENT '每人限领张数',
  `min_point` decimal(10,2) DEFAULT 0.000 COMMENT '使用门槛(最少消费金额才能使用优惠券)；0表示无门槛',
  `use_type` tinyint(1) DEFAULT 0 COMMENT '使用类型：0->全场通用；1->指定分类；2->指定商品',
  `publish_count` int(11) DEFAULT 0 COMMENT '发行数量, 0->无限量',
  `use_count` int(11) DEFAULT 0 COMMENT '已使用数量',
  `receive_count` int(11) DEFAULT 0 COMMENT '领取数量',
  `code` varchar(64) DEFAULT '' COMMENT '优惠码',
  `customer_level` varchar(1200) DEFAULT '' COMMENT '可领取的会员类型主键（用逗号隔开）：空->无限制',
  `pick_start_time` datetime DEFAULT NULL COMMENT '可以领取开始时间',
  `pick_end_time` datetime DEFAULT NULL COMMENT '可以领取结束时间',
  `use_time_type` smallint(6) DEFAULT 0 COMMENT '有效时间限制，如果是0，则基于领取时间的有效天数days；如果是1，则use_start_time和use_end_time是优惠券有效期；',
...
CREATE TABLE `t_customer_coupon_history` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '用户优惠券主键(自增)',
  `account_code` varchar(36) NOT NULL COMMENT '账户code',
  `customer_code` varchar(36) NOT NULL COMMENT '用户code',
  `customer_nick` varchar(36) NOT NULL COMMENT '用户昵称',
  `coupon_id` varchar(36) NOT NULL COMMENT '优惠券主键',
  `order_id` varchar(36) DEFAULT '' COMMENT '订单主键',
  `get_type` tinyint(1) DEFAULT 0 COMMENT '获取类型：0->后台赠送；1->主动获取',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户优惠券使用领取表';
...
CREATE TABLE `t_coupon_product_relation` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '优惠券商品主键(自增)',
  `coupon_id` varchar(36) NOT NULL COMMENT '优惠券主键',
  `product_id` varchar(36) NOT NULL COMMENT '商品主键',
  ...
  KEY `idx_coupon_id` (`coupon_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='优惠券和商品的关系表';
...
CREATE TABLE `t_coupon_product_category_relation` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '优惠券商品主键(自增)',
  `coupon_id` varchar(36) NOT NULL COMMENT '优惠券主键',
  `product_category_id` varchar(36) NOT NULL COMMENT '类目主键',
  `product_category_name` varchar(64) NOT NULL COMMENT '类目名称',
  `enabled` tinyint(1) DEFAULT 0 COMMENT '是否启用 1:启用',
  `created_at` datetime NOT NULL COMMENT '创建时间',
  `updated_at` datetime NOT NULL COMMENT '更新时间',
  `deleted_at` datetime DEFAULT NULL  COMMENT '删除时间',
  ...
  KEY `idx_coupon_id` (`coupon_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='优惠券和商品类目关系表';
