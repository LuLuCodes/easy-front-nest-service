...
@Table({ tableName: "t_spu_customer_price", timestamps: false, comment: "\u5546\u54C1(SPU)\u4F1A\u5458\u4EF7\u683C\u8868" })
export class TSpuCustomerPrice extends Model {
    ...
    @Index({ name: "idx_customer_level_id", using: "BTREE", order: "ASC", unique: false })
    @Index({ name: "idx_product_id", using: "BTREE", order: "ASC", unique: false })
    product_id!: number;
    @Column({ allowNull: true, type: DataType.BIGINT })
    @Index({ name: "idx_customer_level_id", using: "BTREE", order: "ASC", unique: false })
    customer_level_id?: number;
    @Column({ allowNull: true, type: DataType.DECIMAL(10,2), comment: "\u4F1A\u5458\u4EF7\u683C" })
