...
export class TOrderDelivery extends Model {
  ...
  @Column({ type: DataType.STRING(100), comment: '三方发货结果ID' })
  feed_id!: string;
  ...
  @Column({ type: DataType.STRING(100), comment: '跟踪码' })
  track_sn!: string;
  ...
  @Column({ type: DataType.STRING(100), comment: '收货人国家' })
  receiver_country!: string;

  @Column({ type: DataType.STRING(200), comment: '详细地址' })
  receiver_detail_address!: string;

  @Column({ type: DataType.STRING(200), comment: '详细地址2' })
  receiver_detail_address2!: string;

  @Column({ type: DataType.STRING(200), comment: '详细地址3' })
  ...
  @Column({ type: DataType.STRING(500), comment: '收货人省份直辖市' })
  receiver_pcd_desc!: string;

  @Column({ type: DataType.STRING(100), comment: '海关编码' })
  customs_sn!: string;

  @Column({ type: DataType.STRING(100), comment: '增值税号' })
  added_tax!: string;

  @Column({ type: DataType.STRING(100), comment: '欧盟税号' })
  eu_tax!: string;
  ...
  @Column({ type: DataType.STRING(100), comment: '对账人' })
  confirm_user!: string;
  ...
  @Column({ type: DataType.STRING(100), comment: '货单入库人' })
  pack_status_inuser!: string;

  @Column({ allowNull: true, type: DataType.DATE, comment: '货单出库时间' })
  pack_status_outtime?: Date;

  @Column({ type: DataType.STRING(100), comment: '货单出库人' })
  pack_status_outuser!: string;

  @Column({ type: DataType.STRING(500), comment: '国际物流运输方式' })
  inter_transport_code!: string;

  @Column({ type: DataType.STRING(500), comment: '国际物流运输方式' })
  inter_transport_name!: string;

  @Column({ type: DataType.STRING(100), comment: '国际第三方物流快递单号' })
  ...
  @Column({ type: DataType.BIGINT, comment: '物流公司编码', defaultValue: '0' })
  @Index({
  ...
  @Column({ type: DataType.STRING(64), comment: '物流公司名称' })
  ...
  @Column({
    ...
    comment: '易递友换单状态 10成功，其他失败',
    defaultValue: '0',
  })
  direct_order_status?: number;

  @Column({ type: DataType.STRING(255), comment: '易递友换单前原始物流单号' })
  ori_delivery_sn!: string;

  @Column({
    type: DataType.STRING(64),
    comment: '易递友换单前原始物流公司名称',
  })
  ori_delivery_company_name!: string;
