...
@Table({ tableName: "t_flash_promotion_product", timestamps: false, comment: "\u9650\u65F6\u8D2D\u5546\u54C1\u8868" })
export class TFlashPromotionProduct extends Model {
    @Column({ primaryKey: true, autoIncrement: true, type: DataType.BIGINT, comment: "\u5173\u7CFB\u4E3B\u952E(\u81EA\u589E)" })
    ...
    @Index({ name: "idx_flash_promotion_id", using: "BTREE", order: "ASC", unique: false })
    flash_promotion_id!: number;
    @Column({ type: DataType.BIGINT, comment: "\u9650\u65F6\u8D2D\u573A\u6B21\u4E3B\u952E" })
    @Index({ name: "idx_flash_promotion_session_id", using: "BTREE", order: "ASC", unique: false })
    ...
    @Index({ name: "idx_flash_promotion_session_id", using: "BTREE", order: "ASC", unique: false })
    @Index({ name: "idx_product_id", using: "BTREE", order: "ASC", unique: false })
    @Index({ name: "idx_publish_status", using: "BTREE", order: "ASC", unique: false })
    publish_status?: number;
    @Column({ allowNull: true, type: DataType.DECIMAL(10,2), comment: "\u6D3B\u52A8\u4EF7\u683C", defaultValue: "0.00" })
    active_price?: string;
    @Column({ allowNull: true, type: DataType.INTEGER, comment: "\u53C2\u52A0\u6D3B\u52A8\u7684\u5E93\u5B58", defaultValue: "0" })
    active_stock?: number;
    @Column({ allowNull: true, type: DataType.INTEGER, comment: "\u6BCF\u4EBA\u9650\u8D2D\u6570\u636E\u91CF", defaultValue: "0" })
    num_limit?: number;
