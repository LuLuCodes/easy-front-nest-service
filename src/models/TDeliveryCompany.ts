...
export class TDeliveryCompany extends Model {
    @Column({ primaryKey: true, autoIncrement: true, type: DataType.BIGINT, comment: "\u7269\u6D41\u516C\u53F8\u4E3B\u952E" })
    @Index({ name: "PRIMARY", using: "BTREE", order: "ASC", unique: true })
    id?: number;
    @Column({ allowNull: true, type: DataType.INTEGER, comment: "\u5E94\u7528id", defaultValue: "10000" })
    app_id?: number;
    @Column({ allowNull: true, type: DataType.STRING(255), comment: "\u7269\u6D41\u516C\u53F8\u540D\u79F0" })
    company_name?: string;
    @Column({ allowNull: true, type: DataType.STRING(50), comment: "\u7269\u6D41\u516C\u53F8\u7F16\u7801" })
    company_code?: string;
