...
@Table({ tableName: "t_shop_freight_template", timestamps: false, comment: "\u8FD0\u8D39\u914D\u7F6E\u8868" })
export class TShopFreightTemplate extends Model {
    ...
    shop_id!: number;
    @Column({ allowNull: true, type: DataType.STRING(100), comment: "\u6DD8\u5B9D\u8FD0\u8D39\u7F16\u7801" })
    tb_template_id?: string;
    @Column({ allowNull: true, type: DataType.STRING, comment: "\u8FD0\u8D39\u914D\u7F6EJSON" })
    freight_json?: string;
    @Column({ allowNull: true, type: DataType.STRING(255), comment: "\u8FD0\u8D39\u914D\u7F6EJSON_Hash" })
    @Index({ name: "idx_freight_json_hash", using: "BTREE", order: "ASC", unique: false })
